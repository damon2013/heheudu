...
class CreateUserAuthsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('user_auths', function (Blueprint $table) {
            ...
            $table->integer('sms_code')->unsigned()->index()->comment('短信码');
            $table->tinyInteger('verified')->default(0)->comment('是否已经验证');
            
            $table->integer('sign')->unsigned()->index()->comment('签名');
            $table->integer('version_id')->unsigned()->index()->comment('版本号');
            $table->integer('platform_type')->unsigned()->index()->comment('平台类型 1ios 2android');
            $table->timestamps();
        });
    }
