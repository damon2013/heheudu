...
class AuthenticateController extends BaseController
{
    ...
    public function register(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            // 'between:5,16',
            'identifier' => ['required',  'regex:/^1[34578]\d{9}$/'],
            'credential' => ['required', 'min:6'],
            'type' => ['required']

        ], [
            'identifier.required' => '请输入手机号',
            'identifier.regex' => '手机号格式不对',
        ...
        $count = User_Auths::where('identifier', $request->identifier)->count();
        if ($count>0) {
           return $this->respondWithErrors('这个手机号码注册过啦', 401);
        }
        // 验证手机验证码
       if (Cache::has($request->identifier)) {
           $key = Cache::get($request->identifier);
           if ($key != $request->code) {
        ...
        //创建关联表
        $userPhoto = new Users_Photo();
        $userPhoto->user_id = $user->id;
        $userPhoto->save();

        $userHopePatner = new Users_HopePatner();
        $userHopePatner->user_id = $user->id;
        $userHopePatner->save();

        $userLocation = new Users_Location();
        $userLocation->user_id = $user->id;
        $userLocation->save();

        $userIdentity = new Users_Identity();
        $userIdentity->user_id = $user->id;
        $userIdentity->save();

        $userHouse = new User_Big_House();
        $userHouse->user_id = $user->id;
        $userHouse->save();

        $userEdu = new Users_Edu();
        $userEdu->user_id = $user->id;
        $userEdu->save();

        $userJob = new Users_Job();
        $userJob->user_id = $user->id;
        $userJob->save();

        $userCar = new Users_Car();
        $userCar->user_id = $user->id;
        $userCar->save();

        $basicStatus = new Basic_status();
        $basicStatus->user_id = $user->id;
        $basicStatus->save();

        $vipStatus = new Vip_Status();
    ...
    public function login(Request $request)
    {
       ...
       $validator = Validator::make($request->all(), [
        ...
        if ($collectionWeb->contains($request->type)) { // 站内
            ...
            $userAuth = User_Auths::where('identifier', $request->identifier)->whereIn('identity_type', ['username', 'mobile', 'email'])->first();
...
//             $userAuth = User_Auths::where('identifier', $request->identifier)->whereIn('identity_type', ['qq', 'weibo', 'weixin'])->first();
//             if (isset($userAuth)) {
//                 $user_id = $userAuth->user_id;
//             } else {
//                 // 新建用户和第三方授权记录
        ...
        if ($user_id != 0) {
          ...
          //'last_login_time' => Carbon::now()->timestamp
            ...
            return $this->respondWithSuccess([
                ...
                'photo_array'=>Users_Photo::where('user_id', $user->id)->get(),
                'hope_partner' => Users_HopePatner::where('user_id', $user->id)->first(),
                'location' => Users_Location::where('user_id', $user->id)->first(),

                'user_identity' => Users_Identity::where('user_id', $user->id)->first(),
                'user_house' => User_Big_House::where('user_id', $user->id)->first(),
                'user_edu' => Users_Edu::where('user_id', $user->id)->first(),
                'user_job' => Users_Job::where('user_id', $user->id)->first(),
                'user_car' => Users_Car::where('user_id', $user->id)->first(),

                 'basic_status' => Basic_status::where('user_id', $user->id)->first(),
                 'vip_status' => Vip_Status::where('user_id', $user->id)->first()
    ...
    public function sendCheckCode(Request $request)
    {
        $mobile = $request->mobile;
          $validator = Validator::make($request->all(), [
            ...
            'mobile.regex' => '手机号已被注册'
        ]);
        if ($validator->fails()) {
            return $this->respondWithFailedValidation($validator);
        }
        
        $count = User_Auths::where('identifier',$mobile)->count();
        if ($count>0) 
        {
            return $this->respondWithErrors('用户已注册', 403);
        }
        // 获取验证码
...
return $this->respondWithSuccess( $randNum,'验证码发送成功');
       //阿里
        // 配置信息
        // $config = [
        //     'app_key'    => '23764081',
        //     'app_secret' => '4b1f891165428b9f4cb8d8c2a919ab30',
        // ];
        ...
        //         'number' => $randNum
        //     ])
        //     ->setSmsFreeSignName('成双APP')
        //     ->setSmsTemplateCode('SMS_62715186');

        // $resp = $client->execute($req);
        
        // if (isset($resp->result)) 
        // {
        //     return $this->respondWithSuccess($resp->result->model, '验证码发送成功');
        // } 
        // else 
        // {
